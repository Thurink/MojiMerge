...
let state = {
    ...
    ji: STARTING_JI,
    autosave: true,
    settings: { sciNotation: false, freezeEnabled: true, emojiSetKey: 'classic', theme: 'dark' },
    ...
    stats: {
        current: { purchasesPerTier: {}, merges: 0, emojisFrozen: 0, lifetimeJi: 0, jiFrozen: 0, jiPassive: 0, tilesUnlocked: 0, emojisDeleted: 0, highestTier: 0, jiSpentShop: 0, jiSpentUpg: 0, runTimeSec: 0 },
        allTime: { purchasesPerTier: {}, merges: 0, emojisFrozen: 0, lifetimeJi: 0, jiFrozen: 0, jiPassive: 0, tilesUnlocked: 0, emojisDeleted: 0, highestTier: 0, jiSpentShop: 0, jiSpentUpg: 0, runTimeSec: 0 },
    },
    lastTick: Date.now(),
};
...
const el = {
    ji: document.getElementById('ji'), jips: document.getElementById('jips'), message: document.getElementById('message'), prestigeLevel: document.getElementById('prestigeLevel'),
    ...
    prestigeBtn: document.getElementById('prestigeBtn'), prestigeTree: document.getElementById('prestigeTree'), prestigeReadyBanner: document.getElementById('prestigeReadyBanner'),
    fullResetBtn: document.getElementById('fullResetBtn'),
    sciToggle: document.getElementById('sciToggle'), freezeToggle: document.getElementById('freezeToggle'), freezeSetting: document.getElementById('freezeSetting'),
    ...
    freezeStatCur: document.getElementById('freezeStatCur'), freezeStatCur2: document.getElementById('freezeStatCur2'), freezeStatAll: document.getElementById('freezeStatAll'), freezeStatAll2: document.getElementById('freezeStatAll2'), freezeHowto: document.getElementById('freezeHowto'), freezeTimerLabel: document.getElementById('freezeTimerLabel'),
    // Stats bonuses
    bonusPrestige: document.getElementById('bonusPrestige'), bonusMojiPPRow: document.getElementById('bonusMojiPPRow'), bonusMojiPP: document.getElementById('bonusMojiPP'), bonusGlobalRow: document.getElementById('bonusGlobalRow'), bonusGlobal: document.getElementById('bonusGlobal'),
    coresSection: document.getElementById('coresSection'), coresGrid: document.getElementById('coresGrid'),
    // Stats elements
    s_curPrestige: document.getElementById('s_curPrestige'), s_curLifeJi: document.getElementById('s_curLifeJi'), s_curFrozenJi: document.getElementById('s_curFrozenJi'), s_curPassiveJi: document.getElementById('s_curPassiveJi'), s_curMerges: document.getElementById('s_curMerges'), s_curFrozen: document.getElementById('s_curFrozen'), s_curPurchases: document.getElementById('s_curPurchases'), s_curTilesUnlocked: document.getElementById('s_curTilesUnlocked'), s_curDeleted: document.getElementById('s_curDeleted'), s_curHighestTier: document.getElementById('s_curHighestTier'), s_curJiSpentShop: document.getElementById('s_curJiSpentShop'), s_curJiSpentUpg: document.getElementById('s_curJiSpentUpg'), s_curRunTime: document.getElementById('s_curRunTime'),
    s_allLifeJi: document.getElementById('s_allLifeJi'), s_allFrozenJi: document.getElementById('s_allFrozenJi'), s_allPassiveJi: document.getElementById('s_allPassiveJi'), s_allMerges: document.getElementById('s_allMerges'), s_allFrozen: document.getElementById('s_allFrozen'), s_allPurchases: document.getElementById('s_allPurchases'), s_allTilesUnlocked: document.getElementById('s_allTilesUnlocked'), s_allDeleted: document.getElementById('s_allDeleted'), s_allHighestTier: document.getElementById('s_allHighestTier'), s_allJiSpentShop: document.getElementById('s_allJiSpentShop'), s_allJiSpentUpg: document.getElementById('s_allJiSpentUpg'), s_allRunTime: document.getElementById('s_allRunTime'),
};
...
function tierPurchaseCount(t) { const c = state.tierPurchases[t]; return (typeof c === 'number' && isFinite(c)) ? c : 0; }
...
function tierPrice(t) { const base = tierBasePrice(t); const infl = Math.pow(1.04, tierPurchaseCount(t)); const eff = shopEfficiencyFactor(); return Math.round(base * infl * eff); }
...
// PPS & bonuses
function prestigeBonusFactor() { return 1 + 0.01 * (state.prestigeLevel || 0); }
function mojiPPFactor() { return 1 + 0.015 * ((state.mojiPlusPlus || 0) + (state.mojiPPLevel || 0)); }
function globalBoostFactor() { return 1 + 0.01 * (state.globalBoostLevel || 0); }
function coreTierFactor(tier) { const c = (state.emojiCores && state.emojiCores[tier]) || 0; return 1 + 0.01 * c; }
...
function totalPPS() { let sum = 0; const capCount = Math.min(state.unlockedCount, boardMaxCapacity()); for (let i = 0; i < capCount; i++) { const cell = state.board[i]; if (cell && !isFrozen(i)) sum += (getPPS(cell.tier) * tileMultiplier(i) * coreTierFactor(cell.tier) + tierInfusionBonusForTile(i)); } return sum * prestigeBonusFactor() * mojiPPFactor() * globalBoostFactor(); }
...
function normalizeState() {
    ...
    state.settings = state.settings && typeof state.settings === 'object' ? state.settings : { sciNotation: false, freezeEnabled: true, emojiSetKey: 'classic', theme: 'dark' };
...
function load() {
    try {
        ...
        if (raw) {
            ...
            if (data && Array.isArray(data.board)) state = { ...state, ...data };
        }
    } catch (e) {
...
function renderBoard() {
    el.board.innerHTML = ''; const gridCount = boardMaxCapacity(); state.unlockedCount = Math.min(state.unlockedCount, gridCount);
    for (let i = 0; i < gridCount; i++) {
        ...
        if (cell && unlocked) {
            const em = document.createElement('div'); em.className = 'emoji'; em.textContent = getEmojiForTier(cell.tier); const tier = document.createElement('div'); tier.className = 'tier-label'; const isMax = cell.tier === maxTierCap(); tier.textContent = isMax ? `T${cell.tier} • Max` : `T${cell.tier}`; if (isMax) tier.classList.add('max'); const pps = document.createElement('div'); pps.className = 'pps-label'; const eff = isFrozen(i) ? 0 : Math.round((getPPS(cell.tier) * tileMultiplier(i) * coreTierFactor(cell.tier) + tierInfusionBonusForTile(i)) * prestigeBonusFactor() * mojiPPFactor() * globalBoostFactor()); pps.textContent = `${fmtJi(eff)} /s`;
            const sb = document.createElement('div'); sb.className = 'stack-badge'; sb.textContent = stacks > 0 ? `×${Math.pow(2, stacks)} (${stacks})` : '';
            d.appendChild(em); d.appendChild(tier); d.appendChild(pps); d.appendChild(sb);
            // Freeze badge (click to thaw)
            if (isFrozen(i)) {
                const fb = document.createElement('div'); fb.className = 'freeze-badge'; fb.textContent = '❄️'; fb.title = 'Click to thaw'; fb.addEventListener('click', () => thawFrozenAt(i)); d.appendChild(fb);
            }
            d.draggable = true; d.addEventListener('dragstart', dragStart); d.addEventListener('dragover', dragOver); d.addEventListener('drop', drop); d.addEventListener('auxclick', onTileAuxClick); d.addEventListener('dblclick', onTileDoubleClick);
        }
...
// Upgrades
function boardUnlockLevel() { return Math.max(0, state.unlockedCount - START_UNLOCKED); }
...
const SHOP_UPGRADE_BASE_COST = 1000; function shopUpgradeLevel() { return Math.max(0, state.maxBuyTier - 1); } function nextShopTierCost() { return Math.round(SHOP_UPGRADE_BASE_COST * Math.pow(2.02, shopUpgradeLevel())); }
...
function globalBoostMaxLevels() { return 2 * maxTierCap(); }
function nextGlobalBoostCost() { const lvl = state.globalBoostLevel || 0; return Math.round(1600 * Math.pow(1.4, lvl)); }

function renderUpgrades() {
    const items = [];
    const boardMaxed = !canUnlockMore();
    items.push({ key: 'board', title: 'Board', desc: 'Unlock +1 tile', sub: `Capacity ${state.unlockedCount}/${boardMaxCapacity()}`, maxTag: boardMaxed, btn: { id: 'unlockBoardBtn', label: `Unlock (${fmtJi(nextUnlockCost())} Ji)`, disabled: (boardMaxed || state.ji < nextUnlockCost()) } });
    const buyCap = buyableTierCap(); const shopMaxed = state.maxBuyTier >= buyCap;
    items.push({ key: 'shop', title: 'Shop', desc: 'Increase max tier to buy +1 (limit T' + buyCap + ')', sub: `Current max T${state.maxBuyTier}`, maxTag: shopMaxed, btn: { id: 'upgradeShopTierBtn', label: `Upgrade (${fmtJi(nextShopTierCost())} Ji)`, disabled: (state.ji < nextShopTierCost() || shopMaxed) } });
    if ((state.prestigeTree.freezing || 0) >= 1) { items.push({ key: 'unfreeze', title: 'Unfreeze Reward', desc: 'Increase thaw reward multiplier', sub: `Level ${state.unfreezeRewardLevel} • Multiplier ×${(2 * (1 + (state.unfreezeRewardLevel || 0) + (state.prestigeTree.unfreezeBaseBonusLevel || 0))).toFixed(0)}`, maxTag: false, btn: { id: 'upgradeUnfreezeRewardBtn', label: `Upgrade (${fmtJi(nextUnfreezeRewardCost())} Ji)`, disabled: (state.ji < nextUnfreezeRewardCost()) } }); }
    if ((state.prestigeTree.bonusStreams || 0) >= 1) { const streamsMaxed = state.bonusStreamsLevel >= boardMaxCapacity(); items.push({ key: 'streams', title: 'Bonus Streams', desc: 'Add concurrent bonus multipliers (reassign every 5s)', sub: `Level ${state.bonusStreamsLevel}/${boardMaxCapacity()}`, maxTag: streamsMaxed, btn: { id: 'upgradeBonusStreamsBtn', label: `Upgrade (${fmtJi(nextBonusStreamsCost())} Ji)`, disabled: (state.ji < nextBonusStreamsCost() || streamsMaxed) } }); }
    if ((state.prestigeTree.streamStackCap || 0) >= 1) { const stackCapMaxed = state.streamStackCapLevel >= 5; items.push({ key: 'streamstack', title: 'Stream Stack Cap', desc: 'Increase max stacks for Streams', sub: `Level ${state.streamStackCapLevel}/5 • Current cap ${1 + (state.streamStackCapLevel || 0)}`, maxTag: stackCapMaxed, btn: { id: 'upgradeStreamStackCapBtn', label: `Upgrade (${fmtJi(nextStreamStackCapCost())} Ji)`, disabled: (state.ji < nextStreamStackCapCost() || stackCapMaxed) } }); }
    if ((state.prestigeTree.occupiedBias || 0) >= 1) { const biasMaxed = state.occupiedBiasLevel >= 50; items.push({ key: 'bias', title: 'Occupied Bias', desc: 'Increase chance to target tiles with emojis', sub: `Level ${state.occupiedBiasLevel}/50 • Chance ${Math.round(Math.min(1, 0.02 * (state.occupiedBiasLevel || 0)) * 100)}%`, maxTag: biasMaxed, btn: { id: 'upgradeOccupiedBiasBtn', label: `Upgrade (${fmtJi(nextOccupiedBiasCost())} Ji)`, disabled: (state.ji < nextOccupiedBiasCost() || biasMaxed) } }); }
    if ((state.prestigeTree.freezing || 0) >= 1 && state.prestigeTree.unfreezeBaseBonusLevel >= 2) { const reduceMaxed = state.freezeTimerReduceLevel >= 15; items.push({ key: 'freezeReduce', title: 'Freeze Timer Reducer', desc: 'Reduce freeze timer by 1s per tier', sub: `Level ${state.freezeTimerReduceLevel}/15 • Current interval ${Math.max(5, 30 - (state.freezeTimerReduceLevel || 0))}s`, maxTag: reduceMaxed, btn: { id: 'upgradeFreezeReduceBtn', label: `Upgrade (${fmtJi(nextFreezeReduceCost())} Ji)`, disabled: (state.ji < nextFreezeReduceCost() || reduceMaxed) } }); }
    if ((state.prestigeTree.globalBoost || 0) >= 1) { const maxLv = globalBoostMaxLevels(); const lv = state.globalBoostLevel || 0; const boostMaxed = lv >= maxLv; items.push({ key: 'globalBoost', title: 'Global Ji/sec Boost', desc: '+1% Ji/sec per level', sub: `Level ${lv}/${maxLv} • Bonus +${lv}%`, maxTag: boostMaxed, btn: { id: 'upgradeGlobalBoostBtn', label: `Upgrade (${fmtJi(nextGlobalBoostCost())} Ji)`, disabled: (state.ji < nextGlobalBoostCost() || boostMaxed) } }); }

    el.upgradesList.innerHTML = ''; items.forEach(it => { const box = document.createElement('div'); box.className = 'upgrade'; const left = document.createElement('div'); const t = document.createElement('div'); t.className = 'title'; t.textContent = it.title; const d = document.createElement('div'); d.className = 'desc'; d.textContent = it.desc; const s = document.createElement('div'); s.className = 'sub'; s.textContent = it.sub; left.appendChild(t); left.appendChild(d); left.appendChild(s); if (it.maxTag) { const tag = document.createElement('span'); tag.className = 'tag'; tag.textContent = 'Max'; left.appendChild(tag); } box.appendChild(left); if (!it.maxTag) { const btn = document.createElement('button'); btn.id = it.btn.id; btn.type = 'button'; btn.textContent = it.btn.label; btn.disabled = it.btn.disabled; box.appendChild(btn); } el.upgradesList.appendChild(box); });
    const btns = { unlockBoardBtn: upgradeUnlockBoard, upgradeShopTierBtn: upgradeShopTier, upgradeUnfreezeRewardBtn: upgradeUnfreezeReward, upgradeBonusStreamsBtn: upgradeBonusStreams, upgradeStreamStackCapBtn: upgradeStreamStackCap, upgradeOccupiedBiasBtn: upgradeOccupiedBias, upgradeFreezeReduceBtn: upgradeFreezeReduce, upgradeGlobalBoostBtn: upgradeGlobalBoost };
    Object.entries(btns).forEach(([id, fn]) => { const b = document.getElementById(id); if (b) { b.addEventListener('click', fn); b.addEventListener('mousedown', e => e.stopPropagation()); } });
}
...
function upgradeShopTier() { const buyCap = buyableTierCap(); if (state.maxBuyTier >= buyCap) { message(`Shop buy limit reached (T${buyCap}).`); return; } const cost = nextShopTierCost(); if (state.ji < cost) { message('Not enough Ji to upgrade shop tier.'); return; } state.ji -= cost; addJiSpentUpg(cost); state.maxBuyTier += 1; message(`Shop upgraded! You can now buy up to T${state.maxBuyTier} (limit T${buyCap}).`); renderAll(); save(); }
function upgradeBonusStreams() { if (!state.prestigeTree.bonusStreamsUnlocked) { message('Unlock Bonus Streams in the Prestige Tree first.'); return; } const max = boardMaxCapacity(); if ((state.bonusStreamsLevel || 0) >= max) { message('Bonus Streams at maximum.'); return; } const cost = nextBonusStreamsCost(); if (state.ji < cost) { message('Not enough Ji to upgrade Bonus Streams.'); return; } state.ji -= cost; addJiSpentUpg(cost); state.bonusStreamsLevel = (state.bonusStreamsLevel || 0) + 1; message(`Bonus Streams +1 (now ${state.bonusStreamsLevel}).`); renderAll(); save(); }
function upgradeStreamStackCap() { if (!state.prestigeTree.streamStackCapUnlocked) { message('Unlock Stream Stack Cap in the Prestige Tree first.'); return; } if ((state.streamStackCapLevel || 0) >= 5) { message('Stream Stack Cap at maximum.'); return; } const cost = nextStreamStackCapCost(); if (state.ji < cost) { message('Not enough Ji to upgrade Stream Stack Cap.'); return; } state.ji -= cost; addJiSpentUpg(cost); state.streamStackCapLevel = (state.streamStackCapLevel || 0) + 1; message(`Stream Stack Cap increased (now ${1 + (state.streamStackCapLevel || 0)}).`); renderAll(); save(); }
function upgradeOccupiedBias() { if (!state.prestigeTree.occupiedBiasUnlocked) { message('Unlock Occupied Bias in the Prestige Tree first.'); return; } if ((state.occupiedBiasLevel || 0) >= 50) { message('Occupied Bias at maximum.'); return; } const cost = nextOccupiedBiasCost(); if (state.ji < cost) { message('Not enough Ji to upgrade Occupied Bias.'); return; } state.ji -= cost; addJiSpentUpg(cost); state.occupiedBiasLevel = (state.occupiedBiasLevel || 0) + 1; message(`Occupied Bias increased (now ${Math.round(Math.min(1, 0.02 * (state.occupiedBiasLevel || 0)) * 100)}%).`); renderAll(); save(); }
function upgradeUnfreezeReward() { if (!state.prestigeTree.freezingUnlocked) { message('Unlock Freezing in the Prestige Tree first.'); return; } const cost = nextUnfreezeRewardCost(); if (state.ji < cost) { message('Not enough Ji to upgrade Unfreeze Reward.'); return; } state.ji -= cost; addJiSpentUpg(cost); state.unfreezeRewardLevel = (state.unfreezeRewardLevel || 0) + 1; message(`Unfreeze Reward increased (×${(2 * (1 + (state.unfreezeRewardLevel || 0) + (state.prestigeTree.unfreezeBaseBonusLevel || 0))).toFixed(0)}).`); renderAll(); save(); }
function upgradeFreezeReduce() { if (!state.prestigeTree.freezingUnlocked || state.prestigeTree.unfreezeBaseBonusLevel < 2) { message('Requires Unfreeze Base II.'); return; } if ((state.freezeTimerReduceLevel || 0) >= 15) { message('Freeze Timer Reducer at maximum.'); return; } const cost = nextFreezeReduceCost(); if (state.ji < cost) { message('Not enough Ji to upgrade Freeze Timer Reducer.'); return; } state.ji -= cost; addJiSpentUpg(cost); state.freezeTimerReduceLevel = (state.freezeTimerReduceLevel || 0) + 1; message(`Freeze interval reduced (now ${Math.max(5, 30 - (state.freezeTimerReduceLevel || 0))}s).`); renderAll(); save(); ensureFreezeScheduler(true); }
function upgradeGlobalBoost() { if (!state.prestigeTree.globalBoost > 0) { message('Unlock Global Ji/sec Boost in the Prestige Tree first.'); return; } const maxLv = globalBoostMaxLevels(); const lv = state.globalBoostLevel || 0; if (lv >= maxLv) { message('Global Ji/sec Boost at maximum.'); return; } const cost = nextGlobalBoostCost(); if (state.ji < cost) { message('Not enough Ji to upgrade Global Ji/sec Boost.'); return; } state.ji -= cost; addJiSpentUpg(cost); state.globalBoostLevel = lv + 1; message(`Global Ji/sec Boost increased (+${state.globalBoostLevel}%).`); renderAll(); save(); }
...
function onTileClick(e) {
    const idx = Number(e.currentTarget.dataset.index); const unlocked = idx < state.unlockedCount; const cell = state.board[idx]; if (!unlocked) { message('Tile is locked. Unlock more tiles in Upgrades.'); return; } if (isFrozen(idx)) { message('Frozen emojis cannot be moved or merged.'); return; }
    ...
    if (selectedIndex == null) { if (cell != null) { selectedIndex = idx; e.currentTarget.classList.add('selected'); } }
        ...
        if (selectedIndex === idx) { selectedIndex = null; return; }
        if (isFrozen(selectedIndex)) { message('Frozen emojis cannot be moved or merged.'); selectedIndex = null; return; }
        const a = state.board[selectedIndex]; const b = state.board[idx]; const cap = maxTierCap(); if (a && b && a.tier === b.tier) {
            const mergedTier = b.tier; const nextTier = b.tier + 1; if (nextTier > cap) { message(`Tier cap T${cap}. Prestige to raise it.`); selectedIndex = null; renderBoard(); return; }
            state.board[idx] = { tier: nextTier }; updateHighestTier(nextTier); state.board[selectedIndex] = null; addStatsMerge(); maybeGrantEmojiCore(mergedTier); message(`Merged to Tier ${nextTier}!`);
...
function drop(e) { e.preventDefault(); const from = Number(e.dataTransfer.getData('text/plain')); const to = Number(e.currentTarget.dataset.index); if (from === to) return; if (to >= state.unlockedCount) { message('Target tile is locked.'); return; } if (isFrozen(from) || isFrozen(to)) { message('Frozen emojis cannot be moved or merged.'); return; } const a = state.board[from]; const b = state.board[to]; if (!a) return; const cap = maxTierCap(); if (a && b && a.tier === b.tier) { const mergedTier = b.tier; const nextTier = b.tier + 1; if (nextTier > cap) { message(`Tier cap T${cap}. Prestige to raise it.`); return; } state.board[to] = { tier: nextTier }; updateHighestTier(nextTier); state.board[from] = null; addStatsMerge(); maybeGrantEmojiCore(mergedTier); message(`Merged to Tier ${nextTier}!`); } else { state.board[to] = a; state.board[from] = b; message('Moved.'); } renderAll(); save(); }
function onTileAuxClick(e) { if (e.button !== 1) return; const idx = Number(e.currentTarget.dataset.index); const unlocked = idx < state.unlockedCount; const cell = state.board[idx]; if (!unlocked || !cell) return; if (isFrozen(idx)) { message('Frozen emojis cannot be deleted.'); return; } if (confirm('Delete this emoji? This cannot be undone.')) { state.board[idx] = null; addDeleted(); renderAll(); save(); } }
function onTileDoubleClick(e) { try { e.preventDefault(); e.stopPropagation(); const idx = Number(e.currentTarget.dataset.index); if (!isFrozen(idx)) return; thawFrozenAt(idx); } catch (err) { console.warn('dblclick error', err); } }
function thawFrozenAt(idx) { frozenSet.delete(idx); const reward = thawReward(idx); state.ji += reward; addStatsFrozen(reward); selectedIndex = null; message(`Thawed! +${fmtJi(reward)} Ji`); renderAll(); save(); }

function thawReward(idx) { const cell = state.board[idx]; if (!cell) return 0; const base = getPPS(cell.tier) * tileMultiplier(idx) * coreTierFactor(cell.tier); const mult = 2 * (1 + (state.unfreezeRewardLevel || 0) + (state.prestigeTree.unfreezeBaseBonusLevel || 0)); return base * mult; }

// Emoji Core grant (1% per merge) when unlocked
function maybeGrantEmojiCore(tier) {
    if ((state.prestigeTree.emojiCore || 0) < 1) return; if (Math.random() < 0.01) { const cur = state.emojiCores[tier] || 0; if (cur < 100) { state.emojiCores[tier] = cur + 1; message(`🎯 Emoji Core gained for T${tier}! (+1% Ji/sec for this tier, ${state.emojiCores[tier]}/100)`); renderStats(); save(); } }
}
...
function pickIndexWeighted() { const occ = occupiedUnlockedIndices(); const chance = Math.min(1, 0.02 * (state.occupiedBiasLevel || 0)); if (Math.random() < chance && occ.length > 0) { return occ[Math.floor(Math.random() * occ.length)]; } const idxs = allIndices(); return idxs[Math.floor(Math.random() * idxs.length)]; }
function reassignStreams() { const pt = state.prestigeTree || {}; if (!pt.bonusStreamsUnlocked) return; clearAllStacks(); const cap = 1 + (state.streamStackCapLevel || 0); const level = state.bonusStreamsLevel || 0; if (level <= 0) { renderBoard(); return; } for (let s = 0; s < level; s++) { const i = pickIndexWeighted(); if (i != null) { stackCounts[i] = Math.min(cap, (stackCounts[i] || 0) + 1); } } renderBoard(); }
...
function runAutoMerge() {
    const capCount = Math.min(state.unlockedCount, boardMaxCapacity()); const positions = {}; for (let i = 0; i < capCount; i++) { const c = state.board[i]; if (c && !isFrozen(i)) { const t = c.tier; if (!positions[t]) positions[t] = []; positions[t].push(i); } }
    for (const t of Object.keys(positions).map(Number).sort((a, b) => b - a)) { const arr = positions[t]; if (arr.length >= 2) { const i = arr[0], j = arr[1]; const nextTier = t + 1; const cap = maxTierCap(); if (nextTier > cap) continue; state.board[i] = { tier: nextTier }; state.board[j] = null; addStatsMerge(); updateHighestTier(nextTier); maybeGrantEmojiCore(t); message(`Auto‑merged to T${nextTier}.`); renderAll(); save(); return; } }
}
...
function maxTierBonusPerEmoji() {
    ...
    let mult = 0;
    if (pt.maxTierBonusLevel >= 1) mult += 10;
    if (pt.maxTierBonusLevel >= 2) mult += 20;
    if (pt.maxTierBonusLevel >= 3) mult += 30;
    return cap * mult;
}
...
function prestige() {
    ...
    if (!canPrestige()) { message('Prestige requires at least one max‑tier emoji on the board.'); return; }
    ...
    state.stats.current = { purchasesPerTier: {}, merges: 0, emojisFrozen: 0, lifetimeJi: 0, jiFrozen: 0, jiPassive: 0, tilesUnlocked: 0, emojisDeleted: 0, highestTier: 0, jiSpentShop: 0, jiSpentUpg: 0, runTimeSec: 0 };
...
const treeNodes = [
    { id: PrestigeIds.maxTierBonus, level: 1, title: 'Max Tier Bonus I (Additive)', sub: '+10×MaxTier Ji per max-tier emoji upon prestige.', cost: 1, prereqs: [], unlock: () => { state.prestigeTree.maxTierBonusLevel = 1; } },
    { id: PrestigeIds.emojiCoreUnlock, level: 2, title: 'Unlock Emoji Cores', sub: '1% chance on merge to gain an Emoji Core for that tier (+1% Ji/sec per core, up to 100, persists across prestige).', cost: 1, prereqs: [PrestigeIds.maxTierBonus], unlock: () => { state.prestigeTree.emojiCore = 1; } },
    { id: PrestigeIds.globalBoostUnlock, level: 2, title: 'Unlock Global Ji/sec Boost', sub: 'Unlocks an upgrade track with levels = 2× emoji cap; each level adds +1% Ji/sec.', cost: 1, prereqs: [PrestigeIds.maxTierBonus], unlock: () => { state.prestigeTree.globalBoost = 1; } },
    { id: PrestigeIds.freezingUnlock, level: 3, title: 'Unlock Freezing', sub: 'Enables freeze/thaw mechanic and Freeze settings.', cost: 1, prereqs: [PrestigeIds.maxTierBonus], unlock: () => { state.prestigeTree.freezing = 1; ensureFreezeScheduler(true); } },
    { id: PrestigeIds.shopEfficiency1, level: 3, title: 'Shop Efficiency I', sub: 'Reduces tier prices by 1% (cumulative).', cost: 1, prereqs: [PrestigeIds.maxTierBonus], unlock: () => { state.prestigeTree.shopEfficiencyLevel += 1; } },
    { id: PrestigeIds.unfreezeBase1, level: 4, title: 'Unfreeze Base I', sub: '+1 to base thaw multiplier.', cost: 1, prereqs: [PrestigeIds.freezingUnlock], unlock: () => { state.prestigeTree.unfreezeBaseBonusLevel = Math.max(state.prestigeTree.unfreezeBaseBonusLevel || 0, 1); } },
    { id: PrestigeIds.boardStart1, level: 4, title: 'Board Start I', sub: '+1 starting unlocked tile immediately.', cost: 1, prereqs: [PrestigeIds.shopEfficiency1], unlock: () => { state.prestigeTree.boardStartBonusLevel += 1; state.unlockedCount += 1; addTilesUnlocked(1); } },
    { id: PrestigeIds.boardCapacity1, level: 5, title: 'Board Capacity I', sub: 'Unlocks +2 tiles capacity.', cost: 1, prereqs: [PrestigeIds.boardStart1], unlock: () => { state.prestigeTree.boardCapacityBonusLevel += 1; } },
    { id: PrestigeIds.bonusStreamsUnlock, level: 5, title: 'Unlock Bonus Streams', sub: 'Unlocks Bonus Streams upgrade (adds concurrent multipliers; reassign every 5s).', cost: 1, prereqs: [PrestigeIds.unfreezeBase1], unlock: () => { state.prestigeTree.bonusStreams = 1; ensureStreamsScheduler(); return true; } },
    { id: PrestigeIds.unfreezeBase2, level: 6, title: 'Unfreeze Base II', sub: '+1 to base thaw multiplier and unlocks Freeze Timer Reducer.', cost: 1, prereqs: [PrestigeIds.unfreezeBase1], unlock: () => { state.prestigeTree.unfreezeBaseBonusLevel = 2; } },
    { id: PrestigeIds.shopEfficiency2, level: 6, title: 'Shop Efficiency II', sub: 'Reduces tier prices by an additional 1%.', cost: 1, prereqs: [PrestigeIds.boardCapacity1], unlock: () => { state.prestigeTree.shopEfficiencyLevel += 1; } },
    { id: PrestigeIds.maxTierBonus2, level: 7, title: 'Max Tier Bonus II (Additive)', sub: '+20×MaxTier Ji per max-tier emoji upon prestige.', cost: 1, prereqs: [PrestigeIds.bonusStreamsUnlock], unlock: () => { state.prestigeTree.maxTierBonusLevel = 2; } },
    { id: PrestigeIds.passivePps1, level: 7, title: 'Passive PPS I', sub: '+5% Ji/sec global multiplier.', cost: 1, prereqs: [PrestigeIds.shopEfficiency2], unlock: () => { state.prestigeTree.passivePpsLevel = 1; } },
    { id: PrestigeIds.streamStackCapUnlock, level: 8, title: 'Unlock Stream Stack Cap', sub: 'Unlocks Stream Stack Cap upgrade (increases max stacks for Streams). Requires Streams.', cost: 1, prereqs: [PrestigeIds.bonusStreamsUnlock], unlock: () => { state.prestigeTree.streamStackCap = 1; return true; } },
    { id: PrestigeIds.occupiedBiasUnlock, level: 8, title: 'Unlock Occupied Bias', sub: 'Unlocks Occupied Bias (+2% per level chance to target occupied tiles).', cost: 1, prereqs: [PrestigeIds.passivePps1], unlock: () => { state.prestigeTree.occupiedBias = 1; } },
    { id: PrestigeIds.passivePps2, level: 9, title: 'Passive PPS II', sub: '+5% Ji/sec global multiplier.', cost: 1, prereqs: [PrestigeIds.streamStackCapUnlock], unlock: () => { state.prestigeTree.passivePpsLevel = 2; } },
    { id: PrestigeIds.shopEfficiency3, level: 9, title: 'Shop Efficiency III', sub: 'Reduces tier prices by an additional 1%.', cost: 1, prereqs: [PrestigeIds.occupiedBiasUnlock], unlock: () => { state.prestigeTree.shopEfficiencyLevel += 1; } },
    { id: PrestigeIds.boardCapacity2, level: 10, title: 'Board Capacity II', sub: '+2 tiles capacity.', cost: 1, prereqs: [PrestigeIds.passivePps2], unlock: () => { state.prestigeTree.boardCapacityBonusLevel += 1; } },
    { id: PrestigeIds.maxTierBonus3, level: 10, title: 'Max Tier Bonus III (Additive)', sub: '+30×MaxTier Ji per max-tier emoji upon prestige.', cost: 1, prereqs: [PrestigeIds.shopEfficiency3], unlock: () => { state.prestigeTree.maxTierBonusLevel = 3; } },
    { id: PrestigeIds.finalInfusion, level: 11, title: 'Tier Infusion (∞)', sub: 'Emoji gain +tier Ji/sec per level (additive). Requires all prior nodes.', cost: 1, prereqs: [PrestigeIds.boardCapacity2, PrestigeIds.maxTierBonus3], infinite: true, unlock: () => { state.prestigeTree.tierInfusionLevel += 1; } },
];
...
function isNodeUnlocked(id) {
    ...
    if (id === PrestigeIds.maxTierBonus) return (pt.maxTierBonusLevel || 0) >= 1;
    if (id === PrestigeIds.maxTierBonus2) return (pt.maxTierBonusLevel || 0) >= 2;
    if (id === PrestigeIds.maxTierBonus3) return (pt.maxTierBonusLevel || 0) >= 3;
    if (id === PrestigeIds.freezingUnlock) return (pt.freezing || 0) >= 1;
    if (id === PrestigeIds.bonusStreamsUnlock) return (pt.bonusStreams || 0) >= 1;
    if (id === PrestigeIds.streamStackCapUnlock) return (pt.streamStackCap || 0) >= 1;
    if (id === PrestigeIds.occupiedBiasUnlock) return (pt.occupiedBias || 0) >= 1;
    if (id === PrestigeIds.emojiCoreUnlock) return (pt.emojiCore || 0) >= 1;
    if (id === PrestigeIds.globalBoostUnlock) return (pt.globalBoost || 0) >= 1;
    if (id === PrestigeIds.shopEfficiency1) return (pt.shopEfficiencyLevel || 0) >= 1;
    if (id === PrestigeIds.shopEfficiency2) return (pt.shopEfficiencyLevel || 0) >= 2;
    if (id === PrestigeIds.shopEfficiency3) return (pt.shopEfficiencyLevel || 0) >= 3;
    if (id === PrestigeIds.boardStart1) return (pt.boardStartBonusLevel || 0) >= 1;
    if (id === PrestigeIds.passivePps1) return (pt.passivePpsLevel || 0) >= 1;
    if (id === PrestigeIds.passivePps2) return (pt.passivePpsLevel || 0) >= 2;
    if (id === PrestigeIds.unfreezeBase1) return (pt.unfreezeBaseBonusLevel || 0) >= 1;
    if (id === PrestigeIds.unfreezeBase2) return (pt.unfreezeBaseBonusLevel || 0) >= 2;
    if (id === PrestigeIds.boardCapacity1) return (pt.boardCapacityBonusLevel || 0) >= 1;
    if (id === PrestigeIds.boardCapacity2) return (pt.boardCapacityBonusLevel || 0) >= 2;
    if (id === PrestigeIds.finalInfusion) return (pt.tierInfusionLevel || 0) >= 1;
    return false;
}
...
function renderPrestigeTree() {
    ...
    levels.forEach(lvl => {
        ...
        nodes.forEach(n => {
            ...
            node.innerHTML = `<div class="title">${n.title}${n.infinite ? ' (∞)' : ''}</div><div class="sub">${n.sub}</div><div class="cost">Cost: ${n.cost} Moji+</div>`;
...
function renderCores() { const cores = state.emojiCores || {}; const tiers = Object.keys(cores).map(n => Number(n)).filter(t => cores[t] > 0).sort((a, b) => a - b); el.coresGrid.innerHTML = ''; if (tiers.length === 0) { el.coresSection.hidden = true; return; } el.coresSection.hidden = false; tiers.forEach(t => { const box = document.createElement('div'); box.className = 'tierbox'; const emoji = getEmojiForTier(t); const count = cores[t]; box.innerHTML = `<span>${emoji} • T${t}</span><span>${count}/100 (+${count}% Ji/sec)</span>`; el.coresGrid.appendChild(box); }); }
function renderStats() {
    const cur = state.stats.current; const all = state.stats.allTime; const freezeUnlocked = !!state.prestigeTree.freezingUnlocked; el.freezeStatCur.hidden = !freezeUnlocked; el.freezeStatCur2.hidden = !freezeUnlocked; el.freezeStatAll.hidden = !freezeUnlocked; el.freezeStatAll2.hidden = !freezeUnlocked;
    ...
    el.bonusMojiPPRow.hidden = !mojiPPVisible; el.bonusMojiPP.textContent = `+${(((state.mojiPlusPlus || 0) + (state.mojiPPLevel || 0)) * 1.5).toFixed(1)}%`;
    const globalVisible = (state.prestigeTree.globalBoost || 0) >= 1 || (state.globalBoostLevel > 0);
    el.bonusGlobalRow.hidden = !globalVisible; el.bonusGlobal.textContent = `+${(state.globalBoostLevel || 0)}%`;
    renderCores();
}
...
function renderAll() {
    ...
    versionLabel.textContent = 'Version: v' + state.version;
}
function updatePrestigeButtonVisibility() { const visible = canPrestige(); el.prestigeBtn.hidden = !visible; }
...
function fullReset() {
    if (!confirm('Full Reset WILL erase ALL progress including Prestige and Moji+/Moji++. Continue?')) return; if (!confirm('Are you sure? This cannot be undone.')) return; try { localStorage.removeItem('moji-merge-save-v14'); } catch (e) { console.warn('Clear saves failed', e); }
    state = { ji: STARTING_JI, autosave: true, settings: { sciNotation: false, freezeEnabled: true, emojiSetKey: 'classic' }, board: Array(BOARD_ARRAY_SIZE).fill(null), unlockedCount: START_UNLOCKED, maxBuyTier: 1, tierPurchases: {}, bonusStreamsLevel: 0, streamStackCapLevel: 0, occupiedBiasLevel: 0, unfreezeRewardLevel: 0, prestigeLevel: 0, mojiPlus: 0, mojiPlusPlus: 0, mojiPPLevel: 0, autoSpawnUnlocked: false, autoMergeUnlocked: false, spawnSpeedLevel: 0, mergeSpeedLevel: 0, freezeTimerReduceLevel: 0, emojiCores: {}, globalBoostLevel: 0, prestigeTree: { maxTierBonusLevel: 0, freezing: 0, bonusStreams: 0, streamStackCap: 0, occupiedBias: 0, emojiCore: 0, globalBoost: 0, shopEfficiencyLevel: 0, boardStartBonusLevel: 0, passivePpsLevel: 0, boardCapacityBonusLevel: 0, unfreezeBaseBonusLevel: 0, tierInfusionLevel: 0 }, stats: { current: { purchasesPerTier: {}, merges: 0, emojisFrozen: 0, lifetimeJi: 0, jiFrozen: 0, jiPassive: 0, tilesUnlocked: 0, emojisDeleted: 0, highestTier: 0, jiSpentShop: 0, jiSpentUpg: 0, runTimeSec: 0 }, allTime: { purchasesPerTier: {}, merges: 0, emojisFrozen: 0, lifetimeJi: 0, jiFrozen: 0, jiPassive: 0, tilesUnlocked: 0, emojisDeleted: 0, highestTier: 0, jiSpentShop: 0, jiSpentUpg: 0, runTimeSec: 0 } }, lastTick: Date.now() };
...
// Events & init
function exportSave() { try { const data = JSON.stringify(state, null, 2); const blob = new Blob([data], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'moji-merge-save-v14.json'; a.click(); URL.revokeObjectURL(a.href); } catch (e) { message('Export failed.'); } }
...
function init() {
    try {
        ...
        // Attach Add 100M Ji button handler after DOM is ready
        setTimeout(() => {
            const addJiBtn = document.getElementById('addJiBtn');
            if (addJiBtn) {
                addJiBtn.addEventListener('click', function () {
                    state.ji += 100000000;
                    renderAll();
                    save();
                    message('+100,000,000 Ji added!');
                });
            }
